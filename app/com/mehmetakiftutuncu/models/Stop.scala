...
case class Stop(name: String, busId: Int, direction: Direction, location: Location) extends ModelBase {
...
trait StopBase extends Jsonable[Stop] {
  ...
  override def fromJson(json: JsValue): Either[Errors, Stop] = {
    try {
      ...
      val errors = nameErrors ++ busIdErrors ++ directionErrors ++ locationErrors
      ...
      if (errors.nonEmpty) {
        ...
        val stop = Stop(nameAsOpt.get, busIdAsOpt.get, Directions.withName(directionAsOpt.get), errorsOrLocation.right.get)
